...
import {
  demoAssets, demoAccounts, demoTickets, demoMaintenance, demoLicenses,
...
const ITDashboard: React.FC = () => {
  ...
  const activeAssets   = demoAssets.filter(a => a.status === 'active').length;
  const inRepair       = demoAssets.filter(a => a.status === 'in-repair').length;

  const activeAccounts = demoAccounts.filter(a => a.status === 'active').length;
  const suspended      = demoAccounts.filter(a => a.status === 'suspended').length;

  const openTickets    = demoTickets.filter(t => t.status === 'open' || t.status === 'in-progress').length;
  const criticalTkts   = demoTickets.filter(t => t.priority === 'critical' && t.status !== 'resolved' && t.status !== 'closed').length;

  const expiringSoon   = demoLicenses.filter(l => l.status === 'expiring-soon').length;
  const expiredLics    = demoLicenses.filter(l => l.status === 'expired').length;

  const scheduledMaint = demoMaintenance.filter(m => m.status === 'scheduled' || m.status === 'in-progress').length;
  ...
  // ── Recent tickets ────────────────────────────────────────────
  const recentTickets = [...demoTickets].sort(
    (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
  ).slice(0, 5);

  // ── Asset condition summary ───────────────────────────────────
  const conditionMap: Record<string, number> = {};
  demoAssets.forEach(a => { conditionMap[a.condition] = (conditionMap[a.condition] || 0) + 1; });
                  ...
                  {recentTickets.map(t => (
                    <tr key={t.id}>
                      <td>
                        ...
                        <div className="small text-muted">{t.raisedBy}</div>
                      </td>
            ...
            <div className="p-4">
              ...
              <div className="mt-3 pt-3 border-top d-flex justify-content-between">
                {[
                  ...
                  { label: 'Decommissioned', count: demoAssets.filter(a => a.status === 'decommissioned').length, color: '#ef4444' },
                ].map(s => (
                ...
                demoLicenses.filter(l => l.status !== 'active').map(l => (
                  <div key={l.id} className="d-flex align-items-center gap-3 py-2 border-bottom" style={{ borderColor: '#f1f5f9' }}>
                    ...
                    <div className="flex-grow-1">
                      <div className="fw-medium small">{l.softwareName}</div>
                      <div className="text-muted" style={{ fontSize: '0.75rem' }}>Expires: {formatDate(l.expiryDate)}</div>
                    </div>
          ...
          <div className="it-table-card">
            ...
            <div className="table-responsive">
              <table className="table it-table">
                ...
                <tbody>
                  {demoMaintenance.filter(m => m.status !== 'completed').map(m => (
                    <tr key={m.id}>
                      <td><div className="fw-medium">{m.assetName}</div></td>
                      <td><span className="it-category-pill text-capitalize">{m.type}</span></td>
                      <td className="text-muted small">{m.performedBy}</td>
                      <td className="text-muted small">{formatDate(m.scheduledDate)}</td>
